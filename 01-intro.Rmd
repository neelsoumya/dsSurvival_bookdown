...
We used the **bookdown** package [@R-bookdown], R Markdown and **knitr** [@xie2015] for this document. Our package **dsSurvival** [@Banerjeef] uses the **metafor** package for meta-analysis [@Viechtbauer2010].
...
We show this using synthetic data. There are 3 data sets that are held on the same server but can be considered separate.

The cens variable has the event information and the survtime variable has the time information. There is also age and gender information.
...
Cox proportional hazards models. The function calls are shown below:
...
A diagnostic summary is shown below. These diagnostics can allow an analyst to determine if the proportional hazards assumption in Cox proportional hazards models is satisfied. If the p-values shown below are greater than 0.05 for any co-variate, then the proportional hazards assumption is correct for that co-variate.
...
The hazard ratios can then be meta-analyzed by running the following commands:
...
from the dsSurvivalClient function *ds.coxphSLMA()*. 
